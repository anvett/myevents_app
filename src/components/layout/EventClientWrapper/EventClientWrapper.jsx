...
export default function EventClientWrapper({ renderData }) {
  ...
  const petalsEnabled = renderData?.eventMeta?.effects?.petalsRain?.enabled;

  const confettiEnabled = renderData?.eventMeta?.effects?.confettiRain?.enabled;
    ...
    <>
      ...
      {(!introEnabled || isOpen) && (
        <>
          ...
          {/* 🎉 Confetti Effect */}
          {confettiEnabled && <ConfettiEffect />}
