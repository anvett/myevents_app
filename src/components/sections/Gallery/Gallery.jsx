...
//   const introColor = colors?.[textStyles.intro] || colors?.dark || "";
//   const counterColor = colors?.[textStyles.counter] || colors?.secondary || "";

//   const titleSize = textSizes.title || "typo-h1";
//   const introSize = textSizes.intro || "typo-body";
//   const counterSize = textSizes.counter || "typo-label";

//   const navigationButton =
//     colors?.[buttonStyles.navigation] || colors?.buttonPrimary || "";
...
//                 className={`absolute left-3 top-1/2 z-10 -translate-y-1/2 px-3 py-2 rounded-lg shadow-md ${navigationButton}`}
...
//                 className={`absolute right-3 top-1/2 z-10 -translate-y-1/2 px-3 py-2 rounded-lg shadow-md ${navigationButton}`}
...
//                 className="group relative overflow-hidden rounded-2xl text-left shadow-lg focus:outline-none cursor-pointer"
...
//       <ModalBase open={isOpen} onClose={closeModal} colors={colors} modalStyles={config?.modalStyles}>
//         {currentImage ? (
...
//                   type="button"
//                   onClick={showPrev}
//                   className={`cursor-pointer px-4 py-2 rounded-lg ${navigationButton}`}
//                   aria-label="Imagen anterior"
//                 >
//                   Anterior
//                 </button>
...
const Gallery = ({ config = {}, variant = "default", theme = {} }) => {
  ...
  const introColor = colors?.[textStyles.intro] || colors?.dark || "";
  const counterColor = colors?.[textStyles.counter] || colors?.secondary || "";
  const captionColor = colors?.[textStyles.caption] || colors?.dark || "";

  const titleSize = textSizes.title || "typo-h1";
  const introSize = textSizes.intro || "typo-body";
  const counterSize = textSizes.counter || "typo-label";
  const captionSize = textSizes.caption || "typo-body";
        ...
        {currentImage ? (
          <div className="flex w-full flex-col items-center">
              ...
              <div className="relative w-full max-w-5xl overflow-hidden rounded-2xl">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    variants={modalImageVariants}
                    transition={{ duration: 0.35, ease: "easeOut" }}
                    className="relative mx-auto aspect-[4/5] max-h-[68vh] w-full"
                  >
                    <Image
                      src={currentImage.src}
                      alt={currentImage.alt}
            ...
            {currentImage.caption && (
              <p
                className={`${captionSize} ${
                  fonts?.body || "font-body"
                } ${captionColor} mt-5 text-center max-w-3xl`}
              >
                {currentImage.caption}
              </p>
            )}
